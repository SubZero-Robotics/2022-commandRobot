...
ThreeBallUpRun::ThreeBallUpRun(DriveSubsystem* dsubsystem, CargoSubsystem* csubsystem)
    : m_drive{dsubsystem}, m_cargo{csubsystem} {
  AddRequirements({dsubsystem, csubsystem});
}

void ThreeBallUpRun::Initialize() {
  // An 2BallsLowPart1 Trajectory
  frc::Trajectory ThreeBallUp1;
   ...
   ThreeBallUp1 = frc::TrajectoryUtil::FromPathweaverJson(deployDirectoryuno.string());

  frc::Trajectory ThreeBallUp2;
   ...
   ThreeBallUp2 = frc::TrajectoryUtil::FromPathweaverJson(deployDirectorydos.string());
   
  // Reset odometry to the starting pose of the trajectory.
  m_drive->ResetOdometry(ThreeBallUp1.InitialPose());

  // this sets up the command
  frc2::RamseteCommand ThreeBallUp1Command = frc2::RamseteCommand(
      ThreeBallUp1, 
  ...
  // this sets up the command
  frc2::RamseteCommand ThreeBallUp2Command = frc2::RamseteCommand(
      ThreeBallUp2, 
      [this]() { return m_drive->GetPose(); },
      frc::RamseteController(DriveConstants::kRamseteB,
                             DriveConstants::kRamseteZeta),
  ...
  frc2::SequentialCommandGroup* myThreeBallUpAuto = new frc2::SequentialCommandGroup(
    frc2::ParallelRaceGroup( 
      std::move(ThreeBallUp1Command),     
...
bool ThreeBallUpRun::IsFinished() { return finished; }

void ThreeBallUpRun::End(bool interrupted) {
