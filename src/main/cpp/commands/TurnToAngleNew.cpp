...
TurnToAngleNew::TurnToAngleNew(units::degree_t target, 
                             DriveSubsystem* subsystem)
    : m_drive(subsystem), m_angle(target) {
  AddRequirements({subsystem});
}

void TurnToAngleNew::Initialize() {
  ...
  auto exampleTrajectory = frc::TrajectoryGenerator::GenerateTrajectory(
      ...
      frc::Pose2d(0_m, 0_m, frc::Rotation2d(m_angle)),
      // Pass the config
  ...
  frc2::RamseteCommand DriveDistanceCommand = frc2::RamseteCommand(
      exampleTrajectory, 
